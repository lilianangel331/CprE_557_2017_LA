...
uniform sampler2D texture_diffuse; //this is the texture
uniform sampler2D texture_normal; //this is the texture
uniform sampler2D texture_spec; //this is the texture
...
vec4 useLight(Light light, vec4 surfacePostion, vec4 normal_transformed, vec3 normal, Material material )
{
    ...
    // Specular color
    vec3 out_specular_color = material.specular;
...
void main(void)                                                   
{
    ...
    tex_displacement = texture(texture_normal, pass_TexCoord);
    
    vec2 displacement = texture_delta * tex_displacement.xy;

    // This function finds the color component for each texture coordinate. 
    vec4 tex_color =  texture(texture_diffuse, pass_TexCoord + displacement);

    vec4 tex_specular = texture(texture_spec, pass_TexCoord);
